...
func AddAdmin(c *gin.Context) {
	...
	c.JSON(http.StatusOK, storage.Result{})
}
...
func DeleteAdmin(c *gin.Context) {
	...
	if err := dbStorage.Delete(&core.Storage{Model: gorm.Model{ID: uint(id)}}); err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: err.Error()})
		return
	}
	c.JSON(http.StatusOK, storage.Result{})
...
func UpdateAdmin(c *gin.Context) {
	...
	tmp := dbStorage.Get(region.ID, &core.Storage{Model: gorm.Model{ID: input.ID}})
	if tmp.Err != nil {
...
func GetAdmin(c *gin.Context) {
	...
	result := dbStorage.Get(region.ID, &core.Storage{Model: gorm.Model{ID: uint(id)}})
	if result.Err != nil {
		c.JSON(http.StatusInternalServerError, common.Error{Error: result.Err.Error()})
		return
	}
	c.JSON(http.StatusOK, storage.Result{Storage: result.Storage})
}
